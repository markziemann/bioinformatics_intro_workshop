...
shapley <- mclapply(1:nrow(train_data), function(i) {
  iml::Shapley$new(predictor, x.interest = train_data[i, ])
},mc.cores=8)

```

Now compute SHAP values for many samples.

```{r,multishap}

shap_list <- lapply(1:nrow(train_data), function(i) {
  s <- Shapley$new(predictor, x.interest = train_data[i, ])
  data.frame(
    row = i,
...
shap_dfa <- aggregate(. ~ feature,shap_df,FUN=meanabs )
