...
dependancies impact final results [@vallet2022toward].

In order to make the work reproducible, we need to capture the computing environment.
This can be done with virtual machines and containers [@gruning2018practical;@ziemann2023five].
...
If you need R in your image, my recommendation is to start with an image that already has R.
...
Copy the codes below into a file called `Apptainer.def` using `nano` or another text editor.
If you are using git to track your code changes, then change the git address to your repo,
...
Create a tmux session so that your session is not cancelled when disconnecting SSH.
...
Before we launch into analysis we need to provision ourselves an interactive session on the hpc.
...
To execute it directly from the shell, you can run this command, substituting the name of your Rmd file:

```
Rscript -e "rmarkdown::render('w6_workflow.Rmd')"
```

Alternatively, you can open an R command line with `R`, then use the following command, substituting the name
...
From R, the results can be copied to the host filesystem like this:

```
file.copy("myworkflow.html","~/projects/myproject/")
```

From the shell command line, results can be copied like this:

```
cp myworkflow.html ~/projects/myproject
```
