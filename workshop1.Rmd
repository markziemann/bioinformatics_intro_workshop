...
* Help Burnet researchers become more confident in conducting bioinformatics analysis and other
big data tasks.
...
![Image credit: Suparna Ganguly](images/distros.jpeg)
...
You will need access to a Linux (Ubuntu) command line interface for this workshop series.
My recommendation is that we work on Burnet's shared computing cluster (HPC).
The benefit of this is that those systems are administered by dedicated HPC staff, so they can
...
## Navigating your home directory

As an exercise, follow the steps:

1. Create a directory called bioinfo_workshop and `cd` into it.

2. Then create a new directory called "session01" and `cd` into that.

3. Use `nano` to create a text file called mytestfile.txt and put a few lines of text
in it.

4. Try the following commands on the file:
  * cat
  * less
  * wc
  * sort
  * cp (copy the file to another name, like mytestfile2.txt)
  * mv (move the file or change the name)
  * rm (remove the duplicate file)
  * sort
  * du

5. Then use `cd ..` or `cd ~` to return to your home directory.

6. Navigate again to the "session01" directory, but use autocomplete with the `cd` command
For example, from the home directory, type "cd bio" and then hit the tab key, it should
autocomplete the rest of the long name, saving you time and being more precise.
